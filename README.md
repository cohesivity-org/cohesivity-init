...
3. **Adds a descriptive pointer** to an existing `AGENTS.md` or `CLAUDE.md`, so an agent opening this project later knows it uses Cohesivity. It only touches a file that is already there, and creates nothing on its own.

It writes nothing else. It does not touch your README.
